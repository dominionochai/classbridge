...
async def lifespan(_: FastAPI):
    loaders: tuple[tuple[str, Callable[[], Any]], ...] = (
        ("faster-whisper", captions._load_whisper),
        ("MediaPipe Hands", sign_in._load_hands),
        ("PaddleOCR", board_ocr._load_ocr),
        ("LAVIS BLIP2", describe._load_model),
        ("sherpa-onnx", tts._load_tts),
        ("YAMNet", sound_alerts._load_yamnet),
    )
    ...
    for name, loader in loaders:
        try:
            ...
            logging.getLogger("classbridge.models").warning(
                "%s startup initialization failed: %s", name, exc
            )
